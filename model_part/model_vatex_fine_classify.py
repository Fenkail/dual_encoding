...
class Cross_modal(nn.Module):
    def __init__(self):
        ...
        self.linear1 = torch.nn.Linear(in_features=768+2048+768*2+1024+2048+2048,
                                       out_features=2048,)
        self.linear2 = torch.nn.Linear(in_features=2048,
                                       out_features=2,
                                       bias=True)
    ...
    def forward(self, vid_emb, cap_emb, label):
        ...
        linear1_out = self.act(self.linear1(input_tensor))
        dropout_out = self.dropout(linear1_out)
        att_in = dropout_out.unsqueeze(1)
        att_out = self.att(att_in, None)
        att_out = att_out.squeeze(1)
        logits = self.linear2(att_out)
        label = label.cuda()
        logits = logits.cuda()
        return logits,label
...
class Dual_Encoding(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.classification = Cross_modal()
        print(self.text_encoding)
        params += list(self.classification.parameters())
    ...
    def forward_emb(self, videoId, cap_tensor, video_tensor, flag, volatile=False, *args):
        """
